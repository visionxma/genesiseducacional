          ...
          <div className="container">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32, flexWrap: 'wrap', gap: 16 }}>
              ...
              {categories.length > 0 && (
                <select
                  ...
                  style={{ padding: '12px 20px', borderRadius: 0, border: '1px solid var(--site-border)', background: 'var(--site-bg)', fontFamily: 'var(--font-inter)', fontSize: '0.95rem', fontWeight: 500, color: 'var(--site-text-primary)', outline: 'none', cursor: 'pointer', boxShadow: 'var(--site-shadow-sm)' }}>
            ...
            {loading ? (
              ...
              <div style={{ textAlign: 'center', padding: '80px 0', color: 'var(--site-text-tertiary)' }}>
                ...
                {filterCategory && (
                  <button onClick={() => setFilterCategory('')} style={{ marginTop: 12, padding: '10px 20px', background: 'var(--site-primary)', color: 'white', border: 'none', cursor: 'pointer', borderRadius: 0, fontWeight: 600 }}>
                    Ver todos
                  </button>
                )}
              </div>
                    ...
                    <div className="glass-panel" style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden', height: '100%', transition: 'transform 0.2s' }} onMouseEnter={e => (e.currentTarget.style.transform = 'translateY(-3px)')} onMouseLeave={e => (e.currentTarget.style.transform = 'translateY(0)')}>
                      ...
                      <div style={{ height: 180, background: 'var(--site-bg)', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                        {curso.image_url
                          ...
                          : <div style={{ width: 64, height: 64, borderRadius: 0, background: `${color}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', color }}>
                              <BookOpen size={32} strokeWidth={1.5} />
                            </div>
                        }
                        {curso.category && (
                          <div style={{ position: 'absolute', top: 12, left: 12, padding: '4px 10px', background: `${color}20`, color, fontSize: '0.72rem', fontWeight: 700, letterSpacing: '0.05em', borderRadius: 0 }}>
                            {curso.category}
                          </div>
                        )}
                      </div>
